...
float ellipsoid_transit_flux_workgroup_body(int gid, int gsz,
    float2 alpha, float2 beta, ang_params_t p,
...
dual_flux_t ellipsoid_transit_flux_dual_workgroup_body(int gid, int gsz,
    float2 alpha, float2 beta, dual_ang_params_t p,
    float3 ax, float ds, float u1, float u2)
{
    dual_params_t q;
    dual_flux_t ret;
...
kernel void ellipsoid_transit_flux_dual(global const float * restrict time,
    global const float * restrict params, global float * restrict flux,
    global float * restrict dflux)
{
    ...
    dual_flux_t Ival = ellipsoid_transit_flux_dual_workgroup_body(gid, gsz,
        alpha, beta, p, ax, ds, u1, u2);
